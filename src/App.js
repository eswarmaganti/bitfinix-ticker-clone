...
const App = () => {
  ...
  const getTickerData = () => {
    ...
    ws.addEventListener("message", (data) => {
      ...
      const tickerData = JSON.parse(data.data);
      if (typeof tickerData[1] === "object") {
        // console.log(typeof tickerData[1]);
        // console.log(tickerData);

        setState({
          high: tickerData[1][8].toLocaleString(),
          low: tickerData[1][9].toLocaleString(),
          volume: Math.round(
            tickerData[1][7] * tickerData[1][0]
          ).toLocaleString(),
          lastPrice: tickerData[1][6].toLocaleString(),
          dailyChange: tickerData[1][4].toFixed(2),
          dailyChangeRelative:
            Math.round((tickerData[1][5] * 100 + Number.EPSILON) * 100) / 100,
        });
      }
    });
